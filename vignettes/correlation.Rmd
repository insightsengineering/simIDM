...
\]
Expectation and variance of PFS and OS can be derived via the survival functions for PFS and OS, respectively:
\[
...
transitionPwc <- piecewise_exponential(h01 = c(1, 1.3), h02 = c(0.8, 1.5), h12 = c(1, 1),
...
We can estimate the parameters via maximum likelihood (ML), using the log-likelihood based on the counting process notation of @andersen1993:
\[
...
We can estimate the parameters like this:
...
Then, in a final step, we could pass `est` to [`corTrans()`](https://insightsengineering.github.io/simIDM/main/reference/corTrans.html) to compute the PFS-OS correlation.
...
```{r}
corPFSOS(data = simData, transition = transition, bootstrap = TRUE)
```
