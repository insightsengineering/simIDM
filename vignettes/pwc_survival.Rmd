...
vignette: >
  %\VignetteIndexEntry{Piecewise constant hazards overall survival calculation}
...
Say the start points of the constant hazard pieces are $0 \equiv t_{01}^{(1)} < \dotsb < t_{01}^{(k_{01})}$, $k_{01} \geq 1$, for $\lambda_{01}(t)$, with corresponding constant positive hazards $h_{01}^{(1)}, \dotsc, h_{01}^{(k_{01})}$. Obviously we use here the smallest set of pieces, i.e. neighboring hazards are required
