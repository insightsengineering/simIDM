...
vignette: |
  %\VignetteIndexEntry{SimulationEngineMSM: power and type I error calculations}
...
## Introduction
The goal of this vignette is to show with a simple example how the package can be used for planning a study design.  Jointly modeling the endpoints OS and PFS  with the illness-death model has the major advantage that, on the one hand, the correlation of the two endpoints is taken into account properly and, on the other hand, the strong assumption of proportional hazards is not required.
...
Using the multistate model approach implies that the trial planning is based on assumptions on the transition hazards in each arm, i.e. six hazards in total (which induces assumptions on the endpoints OS and PFS).
...
The package provides functions that return  the values of  the OS or PFS survival functions for given transition hazards and time points.
```{r}
...
```
Using the Schoenfeld approximation, a preliminary sample size calculation could be made to get an idea of how many events are needed for 80 \% power. For PFS the hazard ratio is known by specification (= 0.6857), for OS an averaged HR can be calculated, e.g. by using the R package AHR [@AHR]. In our example, we get an average OS HR of 0.7981. The average HR can be derived, for example, in the following way:

```{r eval=FALSE}
library(AHR)
avergHR <- function(data) {
  lastTime <- sort(data$OStime, decreasing = TRUE)[1]
  avgHR <- ahrWKM(lastTime, Surv(OStime, OSevent) ~ trt, data, alpha = 0.5)
  return(avgHR[["hr"]])
}
...
averageHR <- lapply(StudyforAvg, avergHR)
MeanAverageHR <- mean(unlist(averageHR))
```
...
and apply it to OS and PFS in our simulated trials.
...
```
  We estimate the type-I error for each endpoint and the global type-I error by counting the significant tests under $H_0$. The global type-I error is empirically estimated by counting the trials in which either a significant log-rank test is observed for the  OS endpoint or for the PFS endpoint.
