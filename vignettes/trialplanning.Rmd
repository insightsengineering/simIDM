...
hRatioPFS
```
For OS, this is not as straightforward, but for constant transition hazards an averaged HR can be calculated using `avgHRExpOS`:
...
schoenfeldPFS <- getSampleSizeSurvival(lambda2 = hPlPFS, hazardRatio = hRatioPFS, 
...
schoenfeldOS <- getSampleSizeSurvival(lambda2 = hPlPFS, hazardRatio = hRatioOS, 
                                accrualTime = 0, accrualIntensity = 500,
                                maxNumberOfSubjects = 1000, sided = 1,
...
If the global type-I error is lower than 5\%, the critical values used for the log-rank test can be lowered until a significance level close to 5\% is obtained. This could be done, for example, in the following way:
...
empGlobalAlpha <- empSignificant(simTrials = simNull,
                                 ...
                                 criticalOS = criticalOS,
                                 eventNumPFS = eventNumPFS,
                                 eventNumOS = eventNumOS)[[3]]
}
```

If the global type-I error is higher than 5\%, we could use a similar procedure to increase the critical values until we are close to the 5\% threshold.
