...
todo: Consider using list instead of separate transition arguments
```{r}
getClinicalTrials <- function(nRep, nPat, transitionByArm) {
  nArm <- length(transitionByArm)
}
```

todo: Consider using `...` to pass arguments to clinical function
```{r}
...
foo(12, 14235, 3463, 36, 36, 346)
```



```{r prototype}
getClinicalTrials <- function(nRep, nArm, nPat, 
...
todo: consider something with lists like this
